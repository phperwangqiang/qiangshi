...
use think\Db;
use think\Config;
class Cases extends Com
{
	public function index(){
		$this->assign('sign','case');
		return view();
	}
	//视频列表数据
	public function indexajax(){
		if(input('get.p') && input('get.l'))
		{
	        ...
	        $res=Db::name('case')->where($where)->limit($thislimit)->page($thispage)->order('id desc')->select();
	        $arr['code']=0;
	        $arr['data']=$res;
	        $arr['count']=$count;
        }
	...
	public function edit(){
		if(input('get.id')){
			...
			$res=Db::name('case')->where($where)->find();
			$this->assign('res',$res);
			return view();
		}
		else{
	...
	public function editajax(){
		...
		if($id!="" && $name!="" && $description!=""  && $picurl!="")
		{
			...
			if($iscunzai){
				...
				$res=Db::name('case')->where($con)->update($data);
				if($res)
				{
					$arr['code']=200;
					$arr['msg']='修改成功';
	...
	public function del(){
		if(input('post.id')!="")
		{
			...
			$res=Db::name('case')->where($where)->delete();
			if($res)
			{
				$arr['code']=200;
				$arr['msg']='删除成功';
